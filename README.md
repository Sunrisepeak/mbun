...
| Target | Result | Rate |
| --- | ---: | ---: |
| Node.js native tests (`compat/node/test/parallel`) | 3,101 / 4,433 files pass (direct execution) | 70.0% |
| Node.js native tests, excluding files that skip themselves | 3,101 / 3,880 files pass | 79.9% |
| Bun native full corpus (`compat/bun/test`) | 978 / 1,902 files fully green | 51.4% |
| Both corpora combined | 4,079 / 6,335 files | 64.4% |
...
**Bun (978)** is a fresh full run over all 1,902 files on the current binary. **Node.js (3,101)** is the 2026-07-30 full run (3,065) plus +36 verified increments, so the two are *not* same-commit comparable to each other; only the Bun row is a direct measurement.
