...
| Target | Result | Rate |
| --- | ---: | ---: |
| Node.js native tests (`compat/node/test/parallel`) | 3,114 / 4,433 files pass (direct execution) | 70.2% |
| Node.js native tests, excluding files that skip themselves | 3,114 / 3,880 files pass | 80.3% |
| Bun native full corpus (`compat/bun/test`) | 1,015 / 1,902 files fully green | 53.4% |
| Both corpora combined | 4,129 / 6,335 files | 65.2% |
...
**Bun (1,015)** is the 2026-07-31 full run (978) plus +37 verified increments. **Node.js (3,114)** is the 2026-07-30 full run (3,065) plus +49 verified increments, so the two are *not* same-commit comparable to each other; only the Bun row is a direct measurement.

**Both figures went DOWN by 14 when the crypto backend moved to the official `compat.openssl` 3.5.1**, and the reason is worth stating rather than hiding in a delta. The previous backend wrapped a prebuilt that exists only for Linux, which is what blocked a macOS build entirely. Twelve of those fourteen files are not breakage: node's own tests read `if (!hasOpenSSL(3, 5)) skip`, and `hasOpenSSL` reads `process.versions.openssl`. While the runtime reported 3.1.5 those tests skipped and counted as passes; reporting 3.5.1 honestly makes them run, and they fail on algorithms mbun has not wired yet — ML-DSA, ML-KEM, SLH-DSA, raw key objects, WebCrypto wrap/unwrap. Leaving the version string at 3.1.5 would have kept all twelve green while linking 3.5.1, and that option was rejected: a coverage number bought with a false version string measures nothing. The remaining two are real behaviour differences under stricter 3.5 verification.
