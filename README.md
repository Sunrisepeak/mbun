...
| Target | Result | Rate |
| --- | ---: | ---: |
| Bun native test corpus (`compat/bun/test`) | 679 / 1,902 files fully green | 35.7% |
| Bun native tests, test level | 26,853 pass / 15,350 fail of 44,250 run | 60.7% |
...
File-level "green" means every executed test in the file passed; it is stricter than an API checklist and lower than test-level pass rates. Node.js files run directly through mbun (exit 0 = pass) without Node's own harness services, so that figure is honest file-level coverage, not API completion. Details and how to reproduce: [`compat/README.md`](compat/README.md).
