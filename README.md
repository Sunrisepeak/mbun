...
| Target | Result | Rate |
| --- | ---: | ---: |
| Node.js native tests (`compat/node/test/parallel`) | 3,127 / 4,433 files pass (direct execution) | 70.5% |
| Node.js native tests, excluding files that skip themselves | 3,127 / 3,880 files pass | 80.6% |
| Bun native full corpus (`compat/bun/test`) | 1,016 / 1,902 files fully green | 53.4% |
| Both corpora combined | 4,143 / 6,335 files | 65.4% |
...
**Bun (1,016)** is the 2026-07-31 full run (978) plus +38 verified increments. **Node.js (3,127)** is the 2026-07-30 full run (3,065) plus +62 verified increments, so the two are *not* same-commit comparable to each other; only the Bun row is a direct measurement.
