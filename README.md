...
| Target | Result | Rate |
| --- | ---: | ---: |
| Node.js native tests (`compat/node/test/parallel`) | 3,122 / 4,433 files pass (direct execution) | 70.4% |
| Node.js native tests, excluding files that skip themselves | 3,122 / 3,880 files pass | 80.5% |
| Bun native full corpus (`compat/bun/test`) | 1,009 / 1,902 files fully green | 53.0% |
| Both corpora combined | 4,131 / 6,335 files | 65.2% |
...
**Bun (1,009)** is the 2026-07-31 full run (978) plus +31 verified increments. **Node.js (3,122)** is the 2026-07-30 full run (3,065) plus +57 verified increments, so the two are *not* same-commit comparable to each other; only the Bun row is a direct measurement.
