...
| Target | Result | Rate |
| --- | ---: | ---: |
| Node.js native tests (`compat/node/test/parallel`) | 3,089 / 4,433 files pass (direct execution) | 69.7% |
| Node.js native tests, excluding files that skip themselves | 3,089 / 3,880 files pass | 79.6% |
| Bun native full corpus (`compat/bun/test`) | 978 / 1,902 files fully green | 51.4% |
| Both corpora combined | 4,067 / 6,335 files | 64.2% |
...
**Bun (978)** is a fresh full run over all 1,902 files on the current binary. **Node.js (3,089)** is the 2026-07-30 full run (3,065) plus +24 verified increments, so the two are *not* same-commit comparable to each other; only the Bun row is a direct measurement.
