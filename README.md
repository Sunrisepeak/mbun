...
| Target | Result | Rate |
| --- | ---: | ---: |
| Node.js native tests (`compat/node/test/parallel`) | 3,107 / 4,433 files pass (direct execution) | 70.1% |
| Node.js native tests, excluding files that skip themselves | 3,107 / 3,880 files pass | 80.1% |
| Bun native full corpus (`compat/bun/test`) | 999 / 1,902 files fully green | 52.5% |
| Both corpora combined | 4,106 / 6,335 files | 64.8% |
...
**Bun (999)** is the 2026-07-31 full run (978) plus +21 verified increments. **Node.js (3,107)** is the 2026-07-30 full run (3,065) plus +42 verified increments, so the two are *not* same-commit comparable to each other; only the Bun row is a direct measurement.
