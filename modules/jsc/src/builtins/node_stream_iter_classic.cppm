// node:stream/iter payload partition — classic node:stream interop (fromReadable/fromWritable/toReadable/toWritable)
//
// Mechanical 1:1 translation of bun-ref src/js/internal/streams/iter/{classic}.ts
...
export module mbun.jsc.js_builtins:node_stream_iter_classic;

import std;

export namespace mbun::jsc::builtins::detail {

inline constexpr std::string_view kNodeStreamIterClassicJS = R"JS(
(function () {
  const G = globalThis;
  ...
  R.def("internal/streams/iter/classic", function (require, module, exports) {
    ...
    const { validateBackpressure, toUint8Array } = require("internal/streams/iter/utils");
    ...
    const fromReadableCache = new WeakMap();
    ...
    function fromReadable(readable) {
      if (readable == null || typeof readable !== "object") {
        throw $ERR_INVALID_ARG_TYPE("readable", "Readable", readable);
      }
    ...
    function toReadable(source, options = kNullPrototype) {
      if (typeof source?.[SymbolAsyncIterator] !== "function") {
        throw $ERR_INVALID_ARG_TYPE("source", "AsyncIterable", source);
      }

      validateObject(options, "options");
      const { highWaterMark = 64 * 1024, signal } = options;
    ...
    function toReadableSync(source, options = kNullPrototype) {
      if (typeof source?.[SymbolIterator] !== "function") {
        throw $ERR_INVALID_ARG_TYPE("source", "Iterable", source);
      }
    ...
    function fromWritable(writable, options = kNullPrototype) {
      ...
      validateObject(options, "options");
      const { backpressure = "strict" } = options;
      ...
      if (writable.writableObjectMode) {
        throw $ERR_INVALID_STATE("Cannot create a stream/iter Writer from an object-mode Writable");
      }
      ...
      function cleanup(err) {
        ...
        waiters = [];
        for (let i = 0; i < pending.length; i++) {
          pending[i].reject(err ?? $makeAbortError());
        }
        if (!listenersInstalled) return;
      ...
      const writer = {
        ...
        write(chunk) {
          if (!isWritable()) {
            return Promise.reject($ERR_STREAM_WRITE_AFTER_END());
          }
          ...
          if (backpressure === "strict" && isFull()) {
            return Promise.reject(
              $ERR_INVALID_STATE_RangeError(
                "Backpressure violation: buffer is full. " + "Await each write() call to respect backpressure.",
              ),
            );
          }
        ...
        writev(chunks) {
          ...
          if (backpressure === "drop-newest" && isFull()) {
            ...
            for (let i = 0; i < chunks.length; i++) {
              totalBytes += toUint8Array(chunks[i]).byteLength;
            }
            return Promise.resolve();
          }
    ...
    function toWritable(writer) {
      if (typeof writer?.write !== "function") {
        throw $ERR_INVALID_ARG_TYPE("writer", "Writer", writer);
      }
      ...
      const hasWritevSync = hasWritev && typeof writer.writevSync === "function";
      const hasEnd = typeof writer.end === "function";
      const hasEndSync = hasEnd && typeof writer.endSync === "function";
      const hasFail = typeof writer.fail === "function";
      const hasSyncWriteAccepted = typeof writer[kSyncWriteAccepted] === "function";
      ...
      function finishAfterSyncBackpressure(cb) {
        ...
        if (ondrain !== null && ondrain !== undefined) {
          ondrain.then(drained => {
            if (drained === false) {
              cb($ERR_INVALID_STATE_TypeError("Stream closed by consumer"));
              return;
            }
            cb();
          }, cb);
      ...
      function _writev(entries, cb) {
        ...
        try {
          writer.writev(chunks).then(() => cb(), cb);
        } catch (err) {
          cb(err);
        }
      }
