// node:stream/iter payload partition — push() writer/readable pair and duplex()
//
// Mechanical 1:1 translation of bun-ref src/js/internal/streams/iter/{push,duplex}.ts
...
export module mbun.jsc.js_builtins:node_stream_iter_push;

import std;

export namespace mbun::jsc::builtins::detail {

inline constexpr std::string_view kNodeStreamIterPushJS = R"JS(
(function () {
  const G = globalThis;
  ...
  R.def("internal/streams/iter/push", function (require, module, exports) {
    ...
    const { validateAbortSignal, validateInteger } = require("internal/validators");

    const { drainableProtocol, kSyncWriteAccepted, kSyncWriteAcceptedOnFalse } = require("internal/streams/iter/types");
    ...
    class PushQueue {
      ...
      constructor(options = { __proto__: null }) {
        ...
        if (this.#signal) {
          this.#abortHandler = () => {
            this.fail(isError(this.#signal.reason) ? this.#signal.reason : $makeAbortError("Aborted"));
          };
      ...
      canWriteSync() {
        ...
        if (
          (this.#backpressure === "strict" || this.#backpressure === "block") &&
      ...
      async writeAsync(chunks, signal) {
        ...
        if (this.#writerState === "closing") {
          throw $ERR_INVALID_STATE_TypeError("Writer is closing");
        }
        ...
        if (this.#consumerState !== "active") {
          throw this.#consumerState === "thrown" && this.#error
            ? this.#error
            : $ERR_INVALID_STATE_TypeError("Stream closed by consumer");
        }
        ...
        switch (this.#backpressure) {
          case "strict":
            if (this.#pendingWrites.length >= this.#highWaterMark) {
              throw $ERR_INVALID_STATE_RangeError(
          ...
          case "block":
            return this.#createPendingWrite(chunks, signal);
          default:
            throw $ERR_INVALID_STATE("Unexpected: writeSync should have handled non-strict policy");
        }
      }
       ...
       */
        ...
        this.#cleanup();
        this.#rejectPendingWrites($ERR_INVALID_STATE_TypeError("Writer closed"));
      ...
      fail(reason) {
        ...
        this.#error = reason ?? $ERR_INVALID_STATE("Failed");
        this.#cleanup();
      ...
      consumerReturn() {
        ...
        this.#cleanup();
        this.#resolvePendingReads();
        this.#rejectPendingWrites($ERR_INVALID_STATE_TypeError("Stream closed by consumer"));
        // If closing, reject the pending end promise
        if (this.#writerState === "closing" && this.#pendingEnd) {
          this.#pendingEnd.reject($ERR_INVALID_STATE_TypeError("Stream closed by consumer"));
          this.#pendingEnd = null;
        }
    ...
    class PushWriter {
      ...
      writeSync(chunk) {
        ...
        if (!result && this.#queue.backpressurePolicy === "block" && this.#queue.desiredSize === 0) {
      ...
      writevSync(chunks) {
        ...
        if (result === -2) {
          ...
          return Promise.reject(this.#queue.error);
        }
        if (result === -3) {
      ...
      [SymbolAsyncDispose]() {
        ...
        if (state === "closing") {
          ...
          return this.#queue.pendingEndPromise ?? Promise.resolve();
        }
        if (state === "open") {
          this.fail();
        }
  ...
  R.def("internal/streams/iter/duplex", function (require, module, exports) {
    ...
    function duplex(options = { __proto__: null }) {
        ...
        get readable() {
          return {
            ...
            [Symbol.asyncIterator]() {
              const iter = aReadable[Symbol.asyncIterator]();
      ...
      const channelB = {
        ...
        get readable() {
          return {
            __proto__: null,
            [Symbol.asyncIterator]() {
              const iter = bReadable[Symbol.asyncIterator]();
      ...
      if (signal) {
        ...
        if (signal.aborted) {
          ...
          signal.addEventListener("abort", abortBoth, { __proto__: null, once: true });
        }
      }
