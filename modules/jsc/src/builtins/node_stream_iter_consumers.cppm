// node:stream/iter payload partition — bytes/text/arrayBuffer/array/tap/merge/ondrain consumers
//
// Mechanical 1:1 translation of bun-ref src/js/internal/streams/iter/{consumers}.ts
...
export module mbun.jsc.js_builtins:node_stream_iter_consumers;

import std;

export namespace mbun::jsc::builtins::detail {

inline constexpr std::string_view kNodeStreamIterConsumersJS = R"JS(
(function () {
  const G = globalThis;
  ...
  R.def("internal/streams/iter/consumers", function (require, module, exports) {
    ...
    const { drainableProtocol, toAsyncStreamable, toStreamable } = require("internal/streams/iter/types");

    const { isAnyArrayBuffer, isSharedArrayBuffer } = require("node:util/types");
          ...
          const onSettled = (iterator, result) => {
            if (result.done) {
              ...
              iterator.next().then(
                ...
                err => {
                  ready.push({ __proto__: null, error: err });
                  if (waitResolve) {
                    waitResolve();
                    waitResolve = null;
                  }
