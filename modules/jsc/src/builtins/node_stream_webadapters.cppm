// node:stream payload partition — internal/webstreams_adapters (Readable/Writable/Duplex <-> WHATWG streams).
//
// Mechanical 1:1 translation of bun-ref src/js/internal/webstreams_adapters.ts
// (三段法 stage 1): TS types stripped and bun's $-intrinsics lowered onto the
// shims in node_stream_core.cppm; algorithm, branches and error text are the
// blueprint's, not reinvented. Replaces the honest `not implemented` stub that
// stood here — every export below actually moves data.
//
// Three blueprint seams have no mbun equivalent and are lowered, not faked:
//   * `$newRustFunction("node_util_binding.rs", "normalizeEncoding")` (line 22)
//     -> the same algorithm inline; node's internal normalizeEncoding, which
//     node_strdec.cppm also mirrors but does not export off its IIFE.
//   * `$processBindingConstants.zlib` (line 34) -> require("node:zlib").constants,
//     the same table. Safe at this module's load time because the whole module is
//     lazily required by the *Web() conversions, long after boot.
//   * `globalThis.reportError` (line 204) is absent in mbun; the fallback is the
//     nextTick rethrow the blueprint itself uses for this intent elsewhere
//     (e.g. lines 470-472), i.e. surface as uncaught rather than swallow.
//
...
export module mbun.jsc.js_builtins:node_stream_webadapters;

import std;

export namespace mbun::jsc::builtins::detail {

inline constexpr std::string_view kNodeStreamWebAdaptersJS = R"JS(
(function () {
  const G = globalThis;
  ...
  R.def("internal/webstreams_adapters", function (require, module, exports) {
    ...
    const { isAnyArrayBuffer, isPromise } = require("node:util/types");
    const eos = require("internal/streams/end-of-stream");
    const { kEosNodeSynchronousCallback } = eos;

    // bun: $newRustFunction("node_util_binding.rs", "normalizeEncoding", 1).
    // node's internal normalizeEncoding: canonical name, or undefined if unknown.
    ...
    const constants_zlib = require("node:zlib").constants;
    ...
    class ReadableFromWeb extends Readable {
      ...
      _read() {
        __debug("ReadableFromWeb _read()", this.__id);
