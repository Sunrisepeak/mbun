// lockfile/trusted.cppm — mbun.install.lockfile.trusted
//
// DEFAULT_TRUSTED_DEPENDENCIES — mechanical port of bun
// src/install/default-trusted-dependencies.txt (compiled into the package
// manager via lockfile/Package.rs). This is the set of package names bun runs
// lifecycle scripts for without an explicit `trustedDependencies` entry.
export module mbun.install.lockfile.trusted;

import std;

namespace mbun::install::lockfile {

// 367 entries, order preserved from the upstream .txt.
export inline constexpr std::array<std::string_view, 367> DEFAULT_TRUSTED_DEPENDENCIES{{
...
// Linear membership test. Callers that need the on-disk truncated-hash set
// (TruncatedPackageNameHash) build it via the shared wyhash at reconciliation
// time; this is the name-level convenience check.
export constexpr bool is_default_trusted(std::string_view name) noexcept {
    for (auto candidate : DEFAULT_TRUSTED_DEPENDENCIES) {
        if (candidate == name) {
            return true;
        }
    }
